...
def run(
    ...
    if state.get("status") == "waiting_human":
        ...
        if auto_approve:
            user_response = "yes"
            print(f"\nYour response (auto): {user_response}")
        else:
            print("\nYour response: ", end="")
            user_response = input()
...
if __name__ == "__main__":
    ...
    # Scenario 2 — Multi-cloud provisioning
    ...
    # Scenario 5 — Edge extension with full requirements
    ...
    # Scenario 6 — Telemedicine composition (réponse manuelle si waiting_human déclenché)
    #run(
    #    "I am developing a telemedicine application for connected ambulances...",
    #    auto_approve=False,
    #)

    # Scenario 7 — All-in-one orchestration
